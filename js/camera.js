...
let flipCamera     = true; // true = mirror (gương), false = normal
...
function setupCamera() {
  try {
    ...
    video.size(windowWidth, windowHeight); // fullscreen
    video.hide();
    initFaceMesh(video);
...
function drawCamera() {
  ...
  push();
  if (flipCamera) {
    // Mirror: lật ngang — translate sang phải rồi scale -1 theo x
  ...
  // 🔄 Nút flip camera
  push();
  if (flipCamera) { fill(255,77,109,200); }
  else { fill(0,0,0,130); }
  noStroke(); rect(camX+10, camY+44, 80, 26, 13);
  fill(255); textSize(11);
  text(flipCamera ? "🔄 Mirror" : "🔄 Normal", camX+50, camY+57);
  pop();

  // REC 뱃지
  if (mediaRecorder && mediaRecorder.state === "recording") {
    push(); fill(220,50,50,220); noStroke();
    rect(camX+100, camY+10, 58, 26, 13);
    fill(255); textSize(11);
    text("⏺ REC", camX+129, camY+23); pop();
  }
  ...
  // 비디오 저장 버튼
  if (recordedVideoURL) {
    push(); noStroke(); fill(80,180,120,200);
...
function handleCameraButtons() {
  ...
  // Nút flip camera
  if(mouseX>camX+10&&mouseX<camX+90&&
     mouseY>camY+44&&mouseY<camY+70){
...
function takeSinglePhoto() {
  ...
  let timer=setInterval(()=>{
    ...
    if(countdown<=0){
      ...
      // Đợi 1 frame để countdown biến mất trước khi chụp
      setTimeout(()=>{
