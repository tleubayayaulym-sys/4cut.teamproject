...
function applyPhotoFilter(px,py,pw,ph){
  ...
  blendMode(BLEND);
  if(selectedFormat===4){for(let g=0;g<50;g++){stroke(200,180,120,random(15,35));strokeWeight(0.5);point(px+random(pw),py+random(ph));}}
  pop();
}
...
function calcPhotoPositions(sx,sy,sw,ph,pad,gap,L){
  ...
  for(let i=0;i<L.count;i++){
    let px,py;
    if(L.cols===4){px=sx+pad+i*(pw+gap);py=sy+pad;}
    else if(L.cols===2){let col=i%2,row=floor(i/2);px=sx+pad+col*(pw+gap);py=sy+pad+row*(ph+gap);}
    else{px=sx+pad;py=sy+pad+i*(ph+gap);}
...
// ============================================================
// RESULT SCREEN — picapicabooth style
// ============================================================
function drawResultScreen(){
  ...
  // Title
  push();fill(100);textSize(14);noStroke();textFont("Outfit");
  ...
  // Date
  push();noStroke();fill(120);textSize(9);textFont("Outfit");
  let d=new Date();
  ...
  // === RIGHT PANEL — picapicabooth style ===
  // Viền trái nhẹ
  ...
  // Title
  push();fill(30);textSize(min(rw*0.08,18));textFont("Outfit");textStyle(BOLD);noStroke();
  text("Customize your photo strip",rx+rw/2,ry+14);
  textStyle(NORMAL);pop();
  ry+=36;

  // === Frame Colour ===
  push();fill(150);textSize(12);textFont("Outfit");noStroke();textAlign(LEFT,CENTER);
  text("Frame colour",rx,ry+8);pop();
  ry+=22;

  // Pill buttons 2 columns — kiểu picapicabooth
  let pw2=(rw-10)/2, ph2=38, pg=10, pr2=ph2/2;
  for(let i=0;i<frameColors.length;i++){
    ...
    // Shadow
    fill(180,180,200,isSel?40:20);noStroke();
    rect(bx+2,by+3,pw2,ph2,pr2);
    // Nền
    if(isSel){
      // Nền màu frame
      fill(frameColors[i]);
    ...
    rect(bx,by,pw2,ph2,pr2);
    // Text
    noStroke();fill(isSel?frameDark[i]:60);
    textFont("Outfit");textSize(13);textStyle(isSel?BOLD:NORMAL);
  ...
  // Divider
  push();stroke(235);strokeWeight(1);noFill();line(rx,ry,rx+rw,ry);pop();
  ry+=14;

  // === Stickers ===
  push();fill(150);textSize(12);textFont("Outfit");noStroke();textAlign(LEFT,CENTER);
  text("Stickers",rx,ry+8);pop();
  ry+=22;
  ...
  for(let i=0;i<stickerThemeNames.length;i++){
    ...
    fill(180,180,200,isSel?40:15);noStroke();rect(bx+2,by+3,pw2,ph2,pr2);
    fill(isSel?"#fff0f5":255);
    stroke(isSel?"#ff4d6d":210);strokeWeight(isSel?2:1);
    rect(bx,by,pw2,ph2,pr2);
    noStroke();textFont("Outfit");textSize(13);textStyle(isSel?BOLD:NORMAL);
    textAlign(CENTER,CENTER);
  ...
  for(let i=0;i<toneNames2.length;i++){
    ...
    textFont("Outfit");textSize(11);textStyle(isSel?BOLD:NORMAL);
    textAlign(CENTER,CENTER);
  ...
  // Save
  push();noStroke();
  fill(210,80,120,40);rect(rx+2,ry+4,rw,50,25);
  fill(245,75,115);rect(rx,ry,rw,50,25);
  fill(255,130,160,80);rect(rx+2,ry+2,rw-4,24,24,24,0,0);
  noFill();stroke(255,255,255,50);strokeWeight(1);rect(rx+1,ry+1,rw-2,48,25);
  noStroke();fill(255);textFont("Outfit");textStyle(BOLD);textSize(16);
  textAlign(CENTER,CENTER);text("💾  Save Photo",rx+rw/2,ry+25);
  textStyle(NORMAL);pop();
  ry+=62;

  // Retake
  drawLightBtn(rx,ry,rw,40,"🔄  Retake");
...
function handleResultButtons(){
  ...
  let pad=14,gap=5,bot=36,availH=height-60;
  let photoH,stripW;
  if(L.cols===4){let cw=(photoAreaW-60-pad*2-gap*3)/4;photoH=cw*0.75;stripW=pad*2+cw*4+gap*3;}
  else if(L.cols===2){let cw=min((photoAreaW-60-pad*2-gap)/2,availH*0.48/0.75);photoH=cw*0.75;stripW=pad*2+cw*2+gap;}
  else{photoH=(availH-pad*2-gap*(L.rows-1)-bot)/L.rows;let cw=min(photoH/0.75,(photoAreaW-60-pad*2));photoH=cw*0.75;stripW=pad*2+cw;}
  let stripH=L.cols===4?pad*2+photoH+bot:L.cols===2?pad*2+photoH*ceil(L.count/2)+gap*(ceil(L.count/2)-1)+bot:pad*2+photoH*L.count+gap*(L.count-1)+bot;
  ...
  if(mouseX>rx&&mouseX<rx+rw&&mouseY>ry+62&&mouseY<ry+102){
    allPhotos=[];selectedPhotos=[];capturedPhotos=[];selectedSticker=0;currentScreen="camera";
  }
}
...
function saveResultCanvas(){
  ...
  let stripW=L.cols===4?pad*2+cellW*4+gap*3:L.cols===2?pad*2+cellW*2+gap:pad*2+cellW;
  let stripH=L.cols===4?pad*2+photoH+bot:L.cols===2?pad*2+photoH*ceil(L.count/2)+gap*(ceil(L.count/2)-1)+bot:pad*2+photoH*L.count+gap*(L.count-1)+bot;
  ...
  for(let i=0;i<L.count;i++){
    ...
    if(capturedPhotos[i]){g.push();g.imageMode(CORNER);g.image(capturedPhotos[i],px,py,pw,ph);g.pop();applyPhotoFilterSave(g,px,py,pw,ph);}
    else{g.push();g.fill(220);g.noStroke();g.rect(px,py,pw,ph,5);g.pop();}
  }
  let d=new Date();
  let ds=d.getFullYear()+"."+String(d.getMonth()+1).padStart(2,"0")+"."+String(d.getDate()).padStart(2,"0");
  g.noStroke();g.fill(120);g.textSize(9);g.text(ds,stripW/2,stripH-14);
  save(g,"4cut_"+frameNames[selectedFrame]+".png");
  g.remove();currentScreen="saved";
}
