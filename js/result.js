// ============================================================
// result.js — 담당: 마이티투짱
// + Photo filter & Sticker sets: 응웬 바오 담 (Tamy)
// ============================================================
...
// --- Sticker sets (배열 사용) ---
// mỗi sticker: x/y (tỉ lệ 0→1), s (emoji), sz (tỉ lệ size)
let stickerSets = [
  [],  // 0: None
  [    // 1: Girlypop
    {x:0.08,y:0.06,s:"🎀",sz:0.18},{x:0.82,y:0.04,s:"🪞",sz:0.15},
    {x:0.88,y:0.82,s:"🦋",sz:0.16},{x:0.04,y:0.84,s:"✨",sz:0.14},
    {x:0.5, y:0.05,s:"🎀",sz:0.12}
  ],
  [    // 2: Love
    {x:0.08,y:0.08,s:"💕",sz:0.16},{x:0.84,y:0.06,s:"🌸",sz:0.17},
    {x:0.9, y:0.86,s:"💗",sz:0.15},{x:0.05,y:0.88,s:"🌷",sz:0.16},
    {x:0.45,y:0.06,s:"✿", sz:0.14},{x:0.88,y:0.44,s:"💫",sz:0.13}
  ],
  [    // 3: Space
    {x:0.82,y:0.04,s:"🪐",sz:0.20},{x:0.06,y:0.06,s:"⭐",sz:0.16},
    {x:0.88,y:0.84,s:"🌙",sz:0.17},{x:0.05,y:0.82,s:"🚀",sz:0.16},
    {x:0.5, y:0.04,s:"✦", sz:0.12},{x:0.86,y:0.44,s:"💫",sz:0.13}
  ],
  [    // 4: Food
    {x:0.06,y:0.04,s:"🍦",sz:0.18},{x:0.82,y:0.06,s:"☕",sz:0.17},
    {x:0.86,y:0.82,s:"🧁",sz:0.18},{x:0.04,y:0.82,s:"🍓",sz:0.16},
    {x:0.5, y:0.04,s:"🍰",sz:0.14}
  ],
  [    // 5: Vintage Star
    {x:0.06,y:0.06,s:"✦", sz:0.16},{x:0.86,y:0.04,s:"★", sz:0.18},
    {x:0.9, y:0.86,s:"✦", sz:0.15},{x:0.04,y:0.88,s:"✿", sz:0.16},
    {x:0.5, y:0.05,s:"◆", sz:0.12},{x:0.88,y:0.46,s:"✦", sz:0.11}
  ]
];

// ============================================================
// Photo filter — áp dụng overlay màu lên ảnh
// ============================================================
function applyPhotoFilter(px, py, pw, ph) {
  if (selectedPhotoFilter === 0) return;
  push(); noStroke(); blendMode(MULTIPLY);
  if      (selectedPhotoFilter === 1) { fill(255, 200, 100, 60);  rect(px,py,pw,ph); } // Warm
  else if (selectedPhotoFilter === 2) { fill(100, 160, 255, 55);  rect(px,py,pw,ph); } // Cool
  else if (selectedPhotoFilter === 3) { fill(200, 200, 200, 180); rect(px,py,pw,ph); } // B&W
  else if (selectedPhotoFilter === 4) { fill(180, 140, 80,  70);  rect(px,py,pw,ph); } // Vintage
  else if (selectedPhotoFilter === 5) { fill(255, 180, 220, 50);  rect(px,py,pw,ph); } // Dreamy
  blendMode(BLEND);
  // Vintage grain
  if (selectedPhotoFilter === 4) {
    for (let g = 0; g < 60; g++) {
      stroke(200, 180, 120, random(15,40));
      strokeWeight(0.5);
...
function applyPhotoFilterSave(g, px, py, pw, ph) {
  ...
  if      (selectedPhotoFilter === 1) { g.fill(255,200,100,60);  g.rect(px,py,pw,ph); }
  else if (selectedPhotoFilter === 2) { g.fill(100,160,255,55);  g.rect(px,py,pw,ph); }
  else if (selectedPhotoFilter === 3) { g.fill(200,200,200,180); g.rect(px,py,pw,ph); }
  else if (selectedPhotoFilter === 4) { g.fill(180,140,80,70);   g.rect(px,py,pw,ph); }
  else if (selectedPhotoFilter === 5) { g.fill(255,180,220,50);  g.rect(px,py,pw,ph); }
...
function drawResultScreen() {
  ...
  for(let i=0; i<count; i++){
    ...
    if(capturedPhotos[i]){
      ...
      if(selectedSticker>0) drawStickerOverlay(px, py, pw, ph, selectedSticker);
      if(selectedFormat===3){
      ...
      fill(180); textSize(11); text("사진 없음",px+pw/2,py+ph/2); pop();
    }
  }
  ...
  for(let i=0;i<4;i++){
    ...
    textSize(9); text(fmtLabels[i],fx+fw/2,fy+34);
    pop();
  }
  ...
  let stkIcons=["✕","🎀","💕","🪐","🍦","✦"];
  let stkW=min((panelW-16)/stickerSets.length-3, 26);
  for(let i=0;i<stickerSets.length;i++){
    push();
    let sx=panelX+8+i*(stkW+4), sy=stk2Y+28;
...
function handleResultButtons() {
  ...
  let panelX=stripX+stripW+14, panelW=width-panelX-12;
  ...
  for(let i=0;i<filterNames2.length;i++){
    ...
    let fx2=panelX+8+col2*(fltW+fltGap);
    let fy2=flt2Y+28+row2*46;
    if(mouseX>fx2&&mouseX<fx2+fltW&&mouseY>fy2&&mouseY<fy2+38){ selectedPhotoFilter=i; return; }
  }
